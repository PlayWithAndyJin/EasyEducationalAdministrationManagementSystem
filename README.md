...
本项目的 api.py 文件内容参考并引用了<a href="https://github.com/openschoolcn/zfn_api/">openschoolcn的正方教务系统 API 项目</a>，并在此基础上做了适配和优化。

## 适配说明
该程序当前已经适配PC Web端以及移动设备Web端。但是由于教务系统会检测是否使用了自动化程序，会拒绝服务器端的访问，明显的就是输入学号及密码后报错：教务系统挂了。如果可以解决部署到服务器被教务系统拒绝访问的问题，可以联系我，跪了~
...
本系统以我学校的新版正方教务管理系统为例开发。只要你的学校使用的是新版正方教务管理系统（即新版“正方教务系统”Web端），均可直接使用本程序，无需修改核心代码。
...
1. **找到你学校教务系统的网址**  
   ...
   ```
   例如：
   - 我学校教务系统网址为 `https://zfjw.ntit.edu.cn/jwglxt/`
...
> **注意事项：**  
> - 仅支持新版正方教务系统（URL 结构和页面风格与我学校的类似）。
...
## 注意事项
- <strong>请勿将该项目部署到服务器上，因为教务系统会判别是否使用了自动化程序。</strong>
...
本项目遵循 GNU GPL v3 开源协议，详见 [copyright](./LICENSE)。
